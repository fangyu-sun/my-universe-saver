...
const settings = {
  lang: injected.lang || urlParams.get('lang') || 'zh',
...
// Map settings to DOM for visual adaptations
...
document.querySelectorAll('.lang-inline-opt').forEach(btn => {
  btn.addEventListener('click', (e) => {
    ...
    const elementsToFade = [mainCopyEl, locationTimeInfoEl, metaInfoEl];
...
function startBroadcasterSession(initialLat, initialLon, initialCity) {
  ...
  window.updateCoordinatesDynamic = function(freshLat, freshLon, freshCity) {
    currentLat = freshLat;
    currentLon = freshLon;
    if (freshCity) cityString = freshCity;
    fetchDomeCandidates();
    currentDomeIndex = 0;
    renderCurrentCandidate();
  };
...
if (settings.lat !== null && settings.lon !== null && !isNaN(settings.lat) && !isNaN(settings.lon)) {
  const finalCity = settings.city || `${settings.lat.toFixed(2)}, ${settings.lon.toFixed(2)}`;
  startBroadcasterSession(settings.lat, settings.lon, finalCity);
} else {
  showFallback();
}
