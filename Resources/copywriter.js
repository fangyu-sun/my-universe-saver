// Vanilla JS adaptation of user's custom copywriter logic

function getRandomTemplate(templates) {
    return templates[Math.floor(Math.random() * templates.length)];
}

window.generateCopy = function(celestialObj, lang = 'zh') {
    if (!celestialObj) {
    ...
    const distanceMatch = distanceStr ? distanceStr.match(/[\d.]+/) : null;
    ...
    if (isPlanet && planetMap[id]) {
        localizedName = planetMap[id][lang] || localizedName;
        if (localizedDistance) {
            if (lang === 'en') {
                localizedDistance = localizedDistance.replace('亿公里', '00 million km').replace('万公里', '0,000 km').replace('公里', ' km');
            } else if (lang === 'ja') {
                localizedDistance = localizedDistance.replace('亿公里', '億km').replace('万公里', '万km').replace('公里', 'km');
            }
        }
    } else if (starMap[id]) {
        localizedName = starMap[id][lang] || localizedName;
        if (localizedDistance) {
            if (lang === 'en') localizedDistance = localizedDistance.replace('光年', ' light-years');
            else if (lang === 'ja') localizedDistance = localizedDistance.replace('光年', '光年');
        }
    } else if (isSatellite) {
        let cleanName = name ? name.trim() : "";
        if (satMap[cleanName]) {
            localizedName = satMap[cleanName][lang] || localizedName;
        } else if (cleanName.startsWith('STARLINK')) {
            if (lang === 'zh-Hans' || lang === 'zh-Hant') localizedName = cleanName.replace('STARLINK', '星链');
            else if (lang === 'ja') localizedName = cleanName.replace('STARLINK', 'スターリンク');
        }
        if (localizedDistance) {
            if (lang === 'en') localizedDistance = localizedDistance.replace('公里', ' km');
            else if (lang === 'ja') localizedDistance = localizedDistance.replace('公里', 'km');
        }
    } else {
        // HYG Star (constellation mapping)
        isAnonymousStar = true;
        if (con && window.constellations) {
            const conData = window.constellations[con];
            if (conData) {
    ...
    // Convert language codes from the screensaver options to the copywriter format
    const langCode = (lang === 'zh-Hans' || lang === 'zh-Hant') ? 'zh' : lang;
    ...
    if (id === 'Sun') {
        ...
        return getRandomTemplate(templates[langCode]);
    }
    if (id === 'Moon') {
        const templates = {
    ...
    return getRandomTemplate(starTemplates[langCode]);
}
